...
_POLISH_DRAFT_PROMPT = """你是一位资深人力资源总监，请把下面这份团队周报初稿，整理成一份可以直接向上级口头汇报的讲话稿。

## 输出结构（严格按此顺序）

1. **开篇总概**：先用一段话概括本周团队整体情况——重点推动了哪些事、整体进展如何、有无明显风险或需要决策的事项、下阶段重点方向。语言精炼，2-4 句。

2. **一、重点专项工作**：每条工作作为独立一段。

3. **二、年度重点工作**：每条工作作为独立一段。

4. **三、日常常规工作**：只用一两句话概括，点出涉及哪些常规事务即可，不要逐条展开具体内容。

## 每条工作的格式（重要）

每条工作用这种格式输出——"编号. 工作名（负责人）：连贯内容"：

`1. 集团高阶岗位招聘（肖凌华·管理者）：海外CTO岗位已完成交付并入职；集团CFO岗位现有几名候选人待冀总、Letitia面试；集团PRD岗位1人初试已通过，待继续寻访其他候选人。`

要求：
- **编号 + 冒号**：每条以"数字. 工作名（负责人）："开头，冒号后接连贯内容。
- **连贯成文，去掉编号分条**：员工填写的"1、2、3"或"-"等分条，整合成通顺连贯文字，去掉序号和多余换行。
- **同名多人合并，按板块连贯叙述**：同名工作事项由多人负责时合并为一条，把各人的进展按板块/职能串联成连贯的话，**不要用"- 人名：内容"逐条分列**。
- 保留关键信息：数字、进度、结果、下一步、待决策事项；删掉过程性流水账。
- 重点专项工作和年度重点工作要逐条覆盖、不能遗漏；日常常规工作概括即可。
- 大段标题直接写"一、重点专项工作"，标题后不加标点符号。
- 语言正式、简洁、有力，直接输出文字，不要 markdown 标记。"""
